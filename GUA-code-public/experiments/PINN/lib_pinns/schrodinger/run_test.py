...
def run_test(network,dataset_path,device=None):
    ...
    mse_value=float(np.mean(mse))
    relative_l2=np.linalg.norm(prediction[:2].reshape(-1)-ground_truth_data[:2].reshape(-1))/np.linalg.norm(ground_truth_data[:2].reshape(-1))
    return mse_loss,mse,prediction,ground_truth_data,mse_value,float(relative_l2)
