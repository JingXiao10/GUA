...
def training(epochs,trainer:SchrodingerTrainerBasis,
    ...
    for i in range(num_run):
        ...
        network.eval()
        network.to(device)
        mse_loss,_,_,_,mse_value,relative_l2=run_test(network,simulation_path,device=device)
        errors.append(mse_loss)
        write_formal_metrics(trainer,"schrodinger",random_seeds[i],mse_value,relative_l2)
        print("mse_loss real:%.5e, mse_loss image:%.5e, mse_loss all:%.5e" % (mse_loss[0],mse_loss[1],mse_loss[2]))
        print("relative_l2:%.5e" % relative_l2)
