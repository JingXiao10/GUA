...
def training(epochs,trainer:KovasznayTrainerBasis,
    ...
    for i in range(num_run):
        ...
        network.eval()
        network.to(device)
        mse_loss,_,_,relative_l2=run_test(network,n_point=n_validation_point,device=device)
        errors.append(mse_loss)
        write_formal_metrics(trainer,"kovasznay",random_seeds[i],mse_loss,relative_l2)
