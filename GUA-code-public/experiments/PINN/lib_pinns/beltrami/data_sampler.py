...
class BeltramiSamplerBase():
    ...
    def __init__(self,n_internal:int=N_INTERNAL,
        ...
        total_area=x_length*y_length+x_length*z_length+y_length*z_length
        self.n_length=int((n_boundary)/total_area*(y_length*z_length))
        self.n_width=int((n_boundary)/total_area*(x_length*z_length))
...
class BeltramiSamplerLH(BeltramiSamplerBase):
    ...
    def __init__(self, n_internal: int = N_INTERNAL, n_boundary: int = N_BOUNDARY, n_initial: int = N_INITIAL, device: str | torch.device = "cuda:0", update_data: bool = False, seed: int = 21339, x_start: float = X_START, x_end: float = X_END, y_start: float = Y_START, y_end: float = Y_END, z_start: float = Z_START, z_end: float = Z_END, simulation_time: float = SIMULATION_TIME) -> None:
        self.random_engine_boundary=LatinHypercube(d=2,seed=seed)
    ...
    def _get_boundary_points(self):
        with torch.no_grad():
            sample=self.random_engine_boundary.random(n=self.n_length)
            y_length_boundary=torch.tensor(
                sample[:,0],device=self.device,dtype=torch.float32
                )*(self.y_end-self.y_start)+self.y_start
            z_length_boundary=torch.tensor(
                sample[:,1],device=self.device,dtype=torch.float32
                )*(self.z_end-self.z_start)+self.z_start
            x_length_boundary=torch.cat(
                [self.x_start*torch.ones(self.n_length//2,device=self.device),
                 self.x_end*torch.ones(self.n_length-self.n_length//2,device=self.device)])

            sample=self.random_engine_boundary.random(n=self.n_width)
            x_width_boundary=torch.tensor(
                sample[:,0],device=self.device,dtype=torch.float32
                )*(self.x_end-self.x_start)+self.x_start
            z_width_boundary=torch.tensor(
                sample[:,1],device=self.device,dtype=torch.float32
                )*(self.z_end-self.z_start)+self.z_start
            y_width_boundary=torch.cat(
                [self.y_start*torch.ones(self.n_width//2,device=self.device),
                 self.y_end*torch.ones(self.n_width-self.n_width//2,device=self.device)])

            sample=self.random_engine_boundary.random(n=self.n_height)
            x_height_boundary=torch.tensor(
                sample[:,0],device=self.device,dtype=torch.float32
                )*(self.x_end-self.x_start)+self.x_start
            y_height_boundary=torch.tensor(
                sample[:,1],device=self.device,dtype=torch.float32
                )*(self.y_end-self.y_start)+self.y_start
            z_height_boundary=torch.cat(
                [self.z_start*torch.ones(self.n_height//2,device=self.device),
                 self.z_end*torch.ones(self.n_height-self.n_height//2,device=self.device)])
...
class BeltramiSamplerMC(BeltramiSamplerBase):
    ...
    def _get_boundary_points(self):
        with torch.no_grad():
            y_length_boundary=torch.rand(self.n_length,
                device=self.device)*(self.y_end-self.y_start)+self.y_start
            z_length_boundary=torch.rand(self.n_length,
                device=self.device)*(self.z_end-self.z_start)+self.z_start
            x_length_boundary=torch.cat(
                [self.x_start*torch.ones(self.n_length//2,device=self.device),
                 self.x_end*torch.ones(self.n_length-self.n_length//2,device=self.device)])
            
            x_width_boundary=torch.rand(self.n_width,
                device=self.device)*(self.x_end-self.x_start)+self.x_start
            z_width_boundary=torch.rand(self.n_width,
                device=self.device)*(self.z_end-self.z_start)+self.z_start
            y_width_boundary=torch.cat(
                [self.y_start*torch.ones(self.n_width//2,device=self.device),
                 self.y_end*torch.ones(self.n_width-self.n_width//2,device=self.device)])

            x_height_boundary=torch.rand(self.n_height,
                device=self.device)*(self.x_end-self.x_start)+self.x_start
            y_height_boundary=torch.rand(self.n_height,
                device=self.device)*(self.y_end-self.y_start)+self.y_start
            z_height_boundary=torch.cat(
                [self.z_start*torch.ones(self.n_height//2,device=self.device),
                 self.z_end*torch.ones(self.n_height-self.n_height//2,device=self.device)])
