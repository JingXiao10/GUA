...
def run_test(network,
    ...
    relative_l2=np.linalg.norm(prediction.reshape(-1)-dataset.uvwp.reshape(-1))/np.linalg.norm(dataset.uvwp.reshape(-1))
    shape=dataset.mesh_shape+(4,)
    if return_ground_truth:
        return float(mse_value),mse.reshape(shape),prediction.reshape(shape),dataset.uvwp.reshape(shape),float(relative_l2)
    else:
        return float(mse_value),mse.reshape(shape),prediction.reshape(shape),float(relative_l2)
